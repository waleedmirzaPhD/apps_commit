...
static double chi      = -5.0;
static double kon  = 1.0;
...
void LS(hiperlife::FillStructure& fillStr)
{
    ...
    double phi_threshold = 9.0; 
    double threshold_mobility =  1.0 - 0.5*(1+tanh((phin-phi_threshold)));
...
void LS_border(hiperlife::FillStructure& fillStr)
{
    ...
    for (int i = 0; i < eNN; i++)
    {
        ...
        // Hessians
        for (int d = 0; d < pDim; d++)
            lapphi += ue_nodes[DOF*i+0] * hNa[pDim*pDim*i+pDim*d+d];
    }
...
void LS_v(hiperlife::FillStructure& fillStr)
{
    ...
    using ttl::index::K, ttl::index::L;
    // Rename variables
    SubFillStructure& subFill = fillStr["dhand_v"];
    int DOF   = subFill.numDOFs;
    int eNN   = subFill.eNN;
    int nDim  = subFill.nDim;
    int pDim  = subFill.pDim;
    ...
    //if (c0<0.01)
    double alpha_ = (1 + 1000*(1.0 - tanh(10000 * (phi - 5.0))));
...
int main(int argc, char *argv[])
{
    ...
    try
    {
        ...
        hiperProbl->setIntegration("BorderInteg", {"dhand"});
    ...
    try
    {
        ...
        // Set Integration
        hiperProbl_v->setIntegration("Integ", {"dhand_v"});
